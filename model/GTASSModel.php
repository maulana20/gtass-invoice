...
{	
	...
	function addReservationTicketTrain($res)
	{
		$client = &$this->client;
		$host = $this->url;
		...
		$data['page'] = 1;
		$data['pageSize'] = 50;
		$client->setUri($host . '/api/tickettrain-trans/list');
		$client->setParameterPost($data);
		...
		$json = json_encode($data);
		$client->setUri($host . '/api/tickettrain-trans/update?act=add');
		$client->setRawData($json, 'application/json');
		...
		$this->logResponse("log/GTASSReservationTicketTrainAdd.html", $response);
		...
		$data['referral'] = 'tickettrain-trans'; // MANDATORY
		$json = json_encode($data);
		$client->setUri($host . '/api/tickettrain-trans/updateroute?act=add');
		$client->setRawData($json, 'application/json');
		...
		$this->logResponse("log/GTASSReservationTicketTrainSchedule.html", $response);
		...
		$data['tourCode'] = NULL;
		$json = json_encode($data);
		$client->setUri($host . '/api/tickettrain-trans/updatedetail?act=add');
		$client->setRawData($json, 'application/json');
...
// GET DATA
	...
	function isAlreadyResTicketTrain($date, $booking_code)
	{
		$client = &$this->client;
		$host = $this->url;
